...
#$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout[0].value * 100000000' |bc)
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout | to_entries[] | select(.value.value >= 0.15) | .key')

check_cmd "UTXO vout selection" "UTXO_VOUT_INDEX" "$UTXO_VOUT_INDEX"

UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq ".vout[$UTXO_VOUT_INDEX].value * 100000000 | floor")
...
TX_INPUTS=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -c '.vin[0] | {txid, vout, sequence: 1}')
...
TX_OUTPUTS=$(jq -n --arg addr1 "$PAYMENT_ADDRESS" --arg amt1 "$PAYMENT_BTC"  --arg addr2 "$CHANGE_ADDRESS" --arg amt2 "$CHANGE_BTC" '{($addr1): ($amt1 | tonumber), ($addr2): ($amt2 | tonumber)}')
...
# WRITE YOUR SOLUTION BELOW:
#setting any of the transaction's UTXO sequence numbers (which are typically set automatically), so that it's more than 0 and less than 0xffffffff-1 (4294967294).
VERIFY_RBF=$(echo "$DECODED_TX" | jq '.vin[0].sequence < 4294967294')
#VERIFY_RBF2=$(echo "$DECODED_TX" | jq -r '.["bip125-replaceable"]')
echo "-VERIFY_RBF: $VERIFY_RBF"

check_cmd "RBF verification" "VERIFY_RBF" "$VERIFY_RBF"

VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" \
  '.vout[] | select(.scriptPubKey.address == $addr) | .value')
...
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" \
  '.vout[] | select(.scriptPubKey.address == $addr) | .n')
...
# STUDENT TASK: Create the outputs JSON structure
CHILD_OUTPUTS=$(jq -n --arg addr "$CHILD_RECIPIENT" --arg amt "$CHILD_SEND_BTC" '{($addr): ($amt | tonumber)}')
...
bitcoin-cli -regtest decoderawtransaction "$SECONDARY_TX"
...
# Convert BTC to satoshis and subtract the fee
TIMELOCK_AMOUNT=$(echo "($SECONDARY_OUTPUT_VALUE * 100000000) - $TIMELOCK_FEE" | bc | awk '{printf "%.0f", $0}')

check_cmd "Timelock amount calculation" "TIMELOCK_AMOUNT" "$TIMELOCK_AMOUNT"

# Convert to BTC
TIMELOCK_BTC=$(printf "%.8f" "$(echo "scale=8; $CHILD_SEND_AMOUNT / 100000000" | bc)")

# STUDENT TASK: Create the outputs JSON structure
TIMELOCK_RECIPIENT="bcrt1qxhy8dnae50nwkg6xfmjtedgs6augk5edj2tm3e"
TIMELOCK_OUTPUTS=$(jq -n --arg addr "$TIMELOCK_RECIPIENT" --arg amt "$CHILD_SEND_BTC" '{($addr): ($amt | tonumber)}')
