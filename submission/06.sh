# Calculate the total output value of this transaction in satoshis
 raw_tx=""
#bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq '[.vout[].value] | add * 100000000 '
bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq '[.vout[].value] | add * 100000000 | floor | tostring + " satoshis"'

